    ...
    <div class="row text-center mb-3">
        <div class="col-4">
            <div class="{{ $data['database'] ? 'text-success' : 'text-danger' }}">
                ...
                <div class="small">{{ __('Database') }}</div>
                <div class="small fw-bold">{{ $data['database'] ? __('Online') : __('Offline') }}</div>
            </div>
        </div>
        <div class="col-4">
            <div class="{{ $data['cache'] ? 'text-success' : 'text-danger' }}">
                <i class="fas fa-memory fa-2x mb-1"></i>
                <div class="small">{{ __('Cache') }}</div>
                <div class="small fw-bold">{{ $data['cache'] ? __('Working') : __('Failed') }}</div>
            </div>
        </div>
        <div class="col-4">
            <div class="{{ $data['disk_free_percent'] > 20 ? 'text-success' : ($data['disk_free_percent'] > 10 ? 'text-warning' : 'text-danger') }}">
                ...
                <div class="small">{{ __('Disk Space') }}</div>
    ...
    <div class="row">
        ...
        <div class="col-6">
            <div class="text-center p-2 bg-light rounded">
                ...
                <small class="text-muted">{{ __('Activities Today') }}</small>
            </div>
        </div>
    </div>
